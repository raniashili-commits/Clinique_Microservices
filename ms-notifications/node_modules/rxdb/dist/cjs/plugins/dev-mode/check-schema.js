...
function checkSchema(jsonSchema) {
      ...
      indexAsArray.forEach(fieldName => {
        ...
        switch (type) {
          case 'string':
            ...
            if (maxLength > 2048) {
              throw (0, _rxError.newRxError)('SC42', {
                index,
                field: fieldName,
                schema: jsonSchema
              });
  ...
  if (jsonSchema.encrypted) {
     ...
     */
    jsonSchema.encrypted.forEach(propPath => {
      var hasEncryptedParent = jsonSchema.encrypted.some(otherPath => otherPath !== propPath && propPath.startsWith(otherPath + '.'));
      if (hasEncryptedParent) {
        throw (0, _rxError.newRxError)('SC43', {
          field: propPath,
          schema: jsonSchema
        });
      }
