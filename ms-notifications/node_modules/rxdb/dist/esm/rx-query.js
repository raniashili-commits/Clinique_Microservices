...
import { auditTime, mergeMap, filter, map, startWith, distinctUntilChanged, shareReplay } from 'rxjs/operators';
...
export var RxQueryBase = /*#__PURE__*/function () {
  ...
  _proto.remove = async function remove(throwIfMissing) {
    ...
    if (Array.isArray(docs)) {
      ...
      var docsArray = [...docs.values()];
      var _result = await this.collection.bulkRemove(docsArray);
      if (_result.error.length > 0) {
        throw rxStorageWriteErrorToRxError(_result.error[0]);
      }
      var resultMap = new Map();
      _result.success.forEach(doc => resultMap.set(doc.primary, doc));
      return resultMap;
    } else {
...
;
  ...
  return _createClass(RxQueryBase, [{
    ...
    function () {
      if (!this._refCount$) {
        this._refCount$ = new BehaviorSubject(null);
      }
    ...
    get: function () {
      if (!this._$) {
        var changeEventBulks$ = this.collection.eventBulks$.pipe(
        /**
         * Performance shortcut.
         * Changes to local documents are not relevant for the query.
         */
        filter(bulk => !bulk.isLocal));
        var liveQueryUpdateThrottleTime = getLiveQueryUpdateThrottleTime(this.collection);
        var updateTrigger$ = liveQueryUpdateThrottleTime > 0 ? merge(of(null), changeEventBulks$.pipe(auditTime(liveQueryUpdateThrottleTime))) : changeEventBulks$.pipe(
        /**
         * Start once to ensure the querying also starts
         * when there where no changes.
         */
        startWith(null));
        var results$ = updateTrigger$.pipe(
        /**
...
function getLiveQueryUpdateThrottleTime(collection) {
